...
<body data-spy="scroll" data-target="nav">
        ...
        <section id="content-2-1" class="content-block content-2-1 bg-deepocean bg-image-cover" style="background-image:url('<%=request.getContextPath()%>/resources/images/blur2.jpg');">
            <table class="table white bg-sienna-hover"> 
                ...
                <tbody> 
                <c:forEach items="${allBookingsForAdmin}" var="bookings">
                    <tr> 
                       <td>${bookings.userID}</td>
                       <td>${bookings.eventName}</td>
					   <td>${bookings.eventType}</td>
					   ...
					   <td>
						...
						<form method="POST" action="${pageContext.request.contextPath}/ApproveBookingAdmin">
							<input type="hidden" name="BookingID" value="${bookings.getBookingID()}"/> 
							<input type="hidden" name="UserID" value="${bookings.getUserID()}"/>
							<input type="hidden" name="approveAction" value="ApproveBooking"/>
							<input type="hidden" name="EventDate" value="${bookings.getEventDate()}"/>
							<input type="hidden" name="VenueID" value="${bookings.getVenueID()}"/>
							<button class="btn btn-info" type="submit">Approve</button>
						</form>
						
						<form method="POST" action="${pageContext.request.contextPath}/RejectBookingAdmin">
							<input type="hidden" name="BookingID" value="${bookings.getBookingID()}"/> 
							<input type="hidden" name="UserID" value="${bookings.getUserID()}"/>
							<input type="hidden" name="EventDate" value="${bookings.getEventDate()}"/>
							<input type="hidden" name="VenueID" value="${bookings.getVenueID()}"/>
							<input type="hidden" name="rejectAction" value="RejectBooking"/>
